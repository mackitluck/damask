...
const PrivacyPolicy = () => {
  ...
  const privacyPolicy = useSelector((state: any) => state?.cmsPage?.data);

  useEffect(() => {
    dispatch(cmsPageAction(generateQuery({urlKey:'privacy-policy'})))
  }, []);
  ...
  useEffect(() => {
    ...
    privacyPolicy?.component?.forEach((component: any) => {
